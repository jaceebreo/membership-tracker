...
class StaffProfile(Model):
    ...
    roles = ManyToManyField(StaffRole)


class GymBranch(Model):
    location = CharField(verbose_name='Gym Branch Location', max_length=128)
...
class Member(Model):
    ...
    qr_code_key = UUIDField(default=uuid.uuid4, editable=True, unique=True)
    branch = ManyToManyField(GymBranch)
    membership_types = ManyToManyField(MembershipType)
    created_at = DateTimeField(auto_now_add=True)
    photo = ImageField(upload_to='member_photos/', null=True, blank=True)
...
class Invoice(Model):
    ...
    member = ForeignKey(Member,  related_name='related_member_invoices',
                        on_delete=SET_NULL, null=True, blank=True)
    payment_date = DateField(verbose_name="Payment Date",)
    payment_method = CharField(verbose_name="Payment Method",
                               max_length=15, choices=PAYMENT_METHODS, default='CASH')
    payment_amount = DecimalField(
        verbose_name='Payment Amount', decimal_places=2, max_digits=10)

    def __str__(self):
        return f"{self.invoice_num} | {self.member}"


class Billing(Model):
    member = ForeignKey(Member, related_name='related_member_billings',
                        on_delete=SET_NULL,  null=True, blank=True)
    ...
    invoice = ForeignKey(Invoice, related_name='related_invoice_billings',
                         on_delete=SET_NULL,  null=True, blank=True)
